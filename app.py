...
if uploaded_file:
    ...
    # Compute daily delimitation lines
    df['date_only'] = df[datetime_col].dt.normalize()
    unique_dates = df['date_only'].drop_duplicates().to_frame(name='midnight')

    # Base glucose line
    line = alt.Chart(df).mark_line().encode(
        x=alt.X(f"{datetime_col}:T", title="Time"),
        y=alt.Y(f"{glucose_col}:Q", title="Glucose"),
        tooltip=[datetime_col, glucose_col]
    )

    # Rolling average line (solid green)
    avg_line = alt.Chart(df).mark_line(color='orange').encode(
        x=alt.X(f"{datetime_col}:T"),
        y=alt.Y("rolling_avg:Q", title="Rolling Average"),
        tooltip=[datetime_col, alt.Tooltip("rolling_avg:Q", title="1h Avg")]
    )

    # Vertical rules at midnight
    rules = alt.Chart(unique_dates).mark_rule(color='lightgray', strokeWidth=1).encode(
        x=alt.X('midnight:T')
    )

    # Layered chart: rules, raw line, avg line
    chart = alt.layer(rules, line, avg_line).properties(width='container', height=400).interactive()
